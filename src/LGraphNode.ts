...
import { LGraph, serializedLGraph } from "./LGraph";
...
export class LGraphNode {
  ...
  onExecute?(param?: any, options?: any);
  onPropertyChanged?(name, v, previousValue?: any);

  onRemoved?();
  updateOutputData?(origin_slot: number);
  ...
  setOutputData(slot: number, data: any) {
    ...
    var output_info: INodeOutputSlot = this.outputs[slot];
    if (!output_info) {
      return;
    }
   ...
   */
  getInputData(slot: number, force_update: boolean) {
    if (!this.inputs) {
      return;
    } //undefined;
   ...
   */
  getInputDataByName(slot_name: string, force_update: boolean) {
    var slot = this.findInputSlot(slot_name);
    if (slot == -1) {
      return null;
    }
    return slot instanceof INodeInputSlot? this.getInputData(slot, force_update);
  }
   ...
   */
  getInputLink(slot: number): LLink {
    if (!this.inputs) {
      return null;
    }
    if (slot < this.inputs.length) {
   ...
   * @param options
   */
  actionDo(action: string, param, options) {
    options = options || {};
    if (this.onAction) {
  ...
  */
   ...
   */
  findInputSlot(name, returnObj?: any) {
    if (!this.inputs) {
      return -1;
    }
    for (var i = 0, l = this.inputs.length; i < l; ++i) {
   ...
   */
  findOutputSlot(name: string, returnObj?: boolean) {
