...
export class LGraph {
  ...
  runStep(num?: number, do_not_catch_errors?: boolean, limit?: number) {
    ...
    var nodes:LGraphNode[] = this._nodes_executable
  ...
  sendActionToCanvas(action, params?: any) {
    if (!this.list_of_graphcanvas) {
      return;
    }
  ...
  remove(node) {
    ...
    this.setDirtyCanvas(true, true);
    this.afterChange(); //TODO: sure? - almost sure is wrong
    this.change();
   ...
   */
  configure(data: serializedLGraph, keep_old?: boolean) {
    if (!data) {
      return;
    }
