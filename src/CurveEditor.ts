...
export class CurveEditor {
    ...
    onMouseMove(localpos, graphcanvas) {
        ...
        let curvepos: Vector2 = [(localpos[0] - this.margin), (localpos[1] - this.margin)];
        let max_dist = 30 / graphcanvas.ds.scale;
        this.nearest = this.getCloserPoint(curvepos, max_dist);
        let point = points[s];
        if (point) {
    ...
    getCloserPoint(pos: Vector2, max_dist: number) {
        let points = this.points;
        if (!points) return -1;
